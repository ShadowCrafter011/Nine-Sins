...
const init_scale = function() {
  ...
  Scale = Math.sqrt(Math.PI / (2000 * 1000) * canvas.height * canvas.width);

  if (difficulty) {
    difficulty.x = canvas.width / 2;
    difficulty.y = canvas.height / 2;
  }
}

init_scale();
window.addEventListener("resize", init_scale);
...
var audioNight = new Audio("music/Night_of_Nights_Flowering_nights.mp3");
var audioFlandre = new Audio("music/Flandre_s Theme - U.N. Owen was her_.mp3");
var audioDeath = new Audio("music/mixkit-arcade-game-explosion-1699.wav");
